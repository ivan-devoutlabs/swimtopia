...
if ( ! function_exists( 'starter_entry_footer' ) ) :
	...
	function starter_entry_footer() {
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'starter' ) . '</span>', $categories_list ); 
			}

			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'starter' ) );
			if ( $tags_list ) {
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'starter' ) . '</span>', $tags_list ); 
			}
		}
